...
namespace Khagesh2
{
    ...
    {
        public Wall(WallType type, string name, string iD, List<Vertex> vertextIDList, double height/*, double wallThickness*/)
        {
        	Type = type;
            Name = name;
            ID = iD;
	...
	public partial class ThisApplication
	{
		...
		public void ReadBuildingJson()
		{
			...
			string fileName = "C:\\Users\\Khagesh Mahajan\\Downloads\\site 31.json";
...
//                	}
		            	...
		            	foreach (var item in lines)
			            {
			                ...
			                if(data["vertices"] != null)
			                {
			                	...
			                	if (vertexIDList != null && vertexIDList.Count == 2)
				                {
					                ...
					                Wall wall = new Wall(wallType, name, id, vertexList, wallHeight);
		            ...
		            createBuildingRevitFile(building, doc, levelAltitude, altitude, levelsCount, j );
				}
			}
		}
		...
		private void CreateWalls(Document doc,List<Wall> wallList, double levelAltitude, double altitude, Dictionary<WallType, ElementId> wallTypeIdMap)
        {
	      	foreach (var wallItem in wallList)
            {
                ...
                Autodesk.Revit.DB.Wall wall = Autodesk.Revit.DB.Wall.Create(doc, Line.CreateBound(p1, p2), wallTypeIdMap[wallItem.Type], Level.GetNearestLevelId(doc, levelAltitude), wallItem.wallHeight, 0, true, false);
